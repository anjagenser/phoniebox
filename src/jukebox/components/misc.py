...
def get_box_name() -> str:
    """Get the Jukebox box name"""
    return cfg.getn('system', 'box_name', default='Jukebox')


@plugin.register
def set_box_name(name: str) -> str:
    """Set the Jukebox box name and persist to config.

    :param name: New name for the box (non-empty string)
    :return: The name that was set
    """
    name = str(name).strip()
    if not name:
        raise ValueError("Box name cannot be empty")
    cfg.setn('system', 'box_name', value=name)
    cfg.save(only_if_changed=True)
    logger.info(f"Box name set to '{name}'")
    return name
