...
class PulseMonitor(threading.Thread):
    ...
    def __init__(self):
        ...
        # Bluetooth auto-routing state
        # Card index of the BT device that has connected but whose sink hasn't appeared yet
        self._pending_bt_card_index: Optional[int] = None
        # Card index of the BT device we are currently routing audio to
        self._active_bt_card_index: Optional[int] = None
        # Sink we were on before switching to Bluetooth (so we can restore it on disconnect)
        self._previous_sink_name: Optional[str] = None
        ...
        if current_event.facility == 'card' and current_event.t == 'new':
            ...
            for card_info in self._pulse_inst.card_list():
                if card_info.index == current_event.index:
                    # ALSA drivers have 'alsa.card_name', Bluetooth uses 'device.description'
            ...
            if self._toggle_on_connect:
                if card_info.driver == 'module-bluez5-device.c':
                    # Bluetooth headset: the A2DP sink doesn't exist yet at this point.
                    # Record the current sink so we can restore it on disconnect, then
                    # wait for the sink.new event to do the actual switch.
                    self._previous_sink_name = self._pulse_inst.server_info().default_sink_name
                    self._pending_bt_card_index = current_event.index
                    logger.info(f"Bluetooth device '{device_name}' connected; "
                                f"waiting for A2DP sink to appear")
                else:
                    # Non-Bluetooth (ALSA, USB, HifiBerry etc.): switch to configured secondary output
            ...
            if (self._active_bt_card_index is not None
                    and current_event.index == self._active_bt_card_index):
                # The Bluetooth headset we routed audio to has disconnected.
                # PulseAudio moves sink inputs to the fallback automatically, but we
                # want to land on the specific sink we were on before, not whatever PA picks.
                logger.info("Active Bluetooth headset disconnected; restoring previous output")
                self._active_bt_card_index = None
                previous = self._previous_sink_name
                self._previous_sink_name = None
                if previous:
                    pulse_control._set_output_by_name(self._pulse_inst, previous)
                else:
                    pulse_control._set_output(self._pulse_inst, 0)
            elif (self._pending_bt_card_index is not None
                    and current_event.index == self._pending_bt_card_index):
                # BT card removed before its sink ever appeared (e.g. connection rejected)
                self._pending_bt_card_index = None
                self._previous_sink_name = None
                pulse_control._publish_volume(self._pulse_inst)
            else:
                # Non-Bluetooth card removed: return to primary output
                pulse_control._set_output(self._pulse_inst, 0)
                pulse_control._publish_volume(self._pulse_inst)

        elif current_event.facility == 'sink' and current_event.t == 'new':
            # Check whether this new sink belongs to the Bluetooth card we are waiting for.
            # The A2DP sink appears a moment after the card.new event, so we handle it here.
            if self._pending_bt_card_index is not None:
                try:
                    sinks = self._pulse_inst.sink_list()
                    new_sink = next((s for s in sinks if s.index == current_event.index), None)
                    if new_sink is not None and new_sink.card == self._pending_bt_card_index:
                        logger.info(f"Bluetooth A2DP sink '{new_sink.name}' appeared; switching output")
                        self._active_bt_card_index = self._pending_bt_card_index
                        self._pending_bt_card_index = None
                        pulse_control._set_output_by_name(self._pulse_inst, new_sink.name)
                except Exception as e:
                    logger.error(f"Error while handling new Bluetooth sink: {e.__class__.__name__}: {e}")
                    self._pending_bt_card_index = None

        elif current_event.facility == 'sink' and current_event.t == 'change':
            pulse_control._publish_volume(self._pulse_inst)
...
class PulseVolumeControl:
    ...
    def _set_output_by_name(self, pulse_inst: pulsectl.Pulse, sink_name: str):
        """Switch to a sink by name and move all active audio streams to it.

        Used for dynamic sinks (Bluetooth A2DP) whose names aren't known at config time.
        Falls back to primary output if the named sink doesn't exist.
        """
        try:
            sink = pulse_inst.get_sink_by_name(sink_name)
        except Exception as e:
            logger.error(f"Cannot switch to sink '{sink_name}': {e.__class__.__name__}: {e}; "
                         f"falling back to primary output")
            self._set_output(pulse_inst, 0)
            return

        # Cap volume on the target sink before switching
        try:
            volume, _ = self._get_volume_and_mute(pulse_inst, sink_name)
            if volume > self._soft_max_volume:
                self._set_volume(pulse_inst, self._soft_max_volume, sink_name)
        except Exception:
            pass
        ...
        # PulseAudio does not automatically move existing streams to a new default sink
        # (module-stream-restore has restore_device=false, and module-switch-on-connect is
        # intentionally not loaded). Move each sink input explicitly.
        for sink_input in pulse_inst.sink_input_list():
            try:
                pulse_inst.sink_input_move(sink_input.index, sink.index)
            except Exception as e:
                logger.warning(f"Could not move sink input {sink_input.index} to '{sink_name}': {e}")

        self._publish_outputs(pulse_inst)
        self._publish_volume(pulse_inst)
        logger.info(f"Audio sink is now '{sink_name}'")
...
def initialize():
    if pulsectl is None:
        raise RuntimeError(
            "pulsectl is not installed; cannot initialize volume control. "
            "On PipeWire systems install pipewire-pulse so the PulseAudio socket is available, "
            "then reinstall pulsectl."
        )
