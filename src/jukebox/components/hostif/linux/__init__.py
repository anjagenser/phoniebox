...
def get_log_level() -> str:
    """Get the current root logger level as a string (e.g. ``'DEBUG'``, ``'INFO'``)"""
    return logging.getLevelName(logging.getLogger().level)


@plugin.register
def set_log_level(level: str) -> str:
    """Set the root logger level at runtime.

    :param level: One of ``'DEBUG'``, ``'INFO'``, ``'WARNING'``, ``'ERROR'``, ``'CRITICAL'``
    :return: The newly active level name
    """
...
def initialize():
    ...
    card = cfg.setndefault('host', 'wlan_power', 'card', value='wlan0')
    if wlan_power:
        _threading.Thread(target=wlan_disable_power_down, args=[card],
                          daemon=True, name='host.wlan_power_down').start()
    hdmi_off = cfg.setndefault('host', 'rpi', 'hdmi_power_down', value=False)
    if hdmi_off:
        _threading.Thread(target=hdmi_power_down,
                          daemon=True, name='host.hdmi_power_down').start()
