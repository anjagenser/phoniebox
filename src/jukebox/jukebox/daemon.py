...
class JukeBox:
    def __init__(self, configuration_file: str, write_artifacts: bool):
        ...
        # Fetch git state in background — two subprocess.run() calls that can take 100–300 ms
        self._git_state = "pending"

        def _fetch_git_state():
            self._git_state = jukebox.utils.get_git_state()
            logger.info(f"Git state: {self._git_state}")

        self._git_state_thread = threading.Thread(target=_fetch_git_state, daemon=True, name="git.state")
        self._git_state_thread.start()
    ...
    def run(self):
        ...
        self._git_state_thread.join(timeout=2.0)
