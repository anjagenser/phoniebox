...
_jukebox_core_install_os_dependencies() {
  ...
  local apt_packages
  apt_packages=$(get_args_from_file "${INSTALLATION_PATH}/packages-core.txt")

  # On Debian 13+ (Trixie) PipeWire is the default audio daemon.
  # PipeWire-Pulse provides full PulseAudio API compatibility (pulsectl, pulsesink, pactl)
  # so no Python/config changes are needed. Standalone pulseaudio would conflict.
  if [[ "$(is_debian_version_at_least 13)" == "true" ]]; then
      apt_packages=$(printf '%s\n' $apt_packages \
          | grep -vE '^pulseaudio$|^pulseaudio-module-bluetooth$' \
          | tr '\n' ' ')
      apt_packages="${apt_packages}pipewire-pulse wireplumber"
  fi
...
_jukebox_core_configure_pulseaudio() {
  # On Trixie, PipeWire-Pulse runs in place of standalone PulseAudio and does not
  # use default.pa — nothing to configure.
  if [[ "$(is_debian_version_at_least 13)" == "true" ]]; then
      log "  Skipping PulseAudio config (PipeWire active on Trixie)"
      return
  fi
...
_jukebox_core_precompile_bytecode() {
  print_lc "  Pre-compile Python bytecode for faster startup"
  python3 -m compileall -q "${INSTALLATION_PATH}/src/jukebox"
}
