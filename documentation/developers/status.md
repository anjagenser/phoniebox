...
- [x] Enable/Disable debug logging from RPC (`host.set_log_level / get_log_level`)
...
- [x] Method to change configuration through WebUI
  - Implemented as typed RPC setters per feature (not a generic YAML editor)
  - Each setter writes to `jukebox.yaml` via `cfg.save()` and applies the change live
  - Implemented: **box name** (`misc.set_box_name`), **second-swipe action** (`player.ctrl.set_second_swipe_option`)
  - The SecondSwipe settings card now loads the current value and persists changes without restart
...
- [x] Folder configuration
  ...
  - [x] Repeat Playlist (loop)
  - [x] Repeat Song (single)
  - [x] Shuffle
  - Config is stored per-folder in `music_player_status.json` under `folder_config`
  - RPC: `player.ctrl.get_folder_config(folder)` / `player.ctrl.set_folder_config(folder, ...)`
  - WebUI: Toggles appear in the card registration form when a folder is selected
...
- [x] Installation routine (`setup_mopidy.sh`) installs Mopidy, Mopidy-Spotify, Mopidy-MPD, Mopidy-Iris and gst-plugin-spotify
- [x] Mopidy replaces MPD when Spotify is enabled (Mopidy implements the MPD protocol, so `playermpd` works unchanged)
- [x] `play_uri` RPC command added to `playermpd` — plays any Mopidy URI (Spotify tracks, albums, playlists)
  - Accepts `spotify:album/track/playlist:id` URIs and `https://open.spotify.com/...` URLs
- [x] Installer prompts for Spotify `client_id` and `client_secret` during setup
- [x] WebUI: Spotify URI input field when registering a card
- [ ] WebUI: Cover art for Spotify content (via Mopidy HTTP JSON-RPC API on port 6680)
...
- [x] Settings configuration page
  - Box name (`misc.set_box_name` RPC + WebUI text field)
  - Second-swipe action (`player.ctrl.set_second_swipe_option` RPC + WebUI radio group)
  - App settings (`misc.set_app_settings` — cover art toggle; persistence bug fixed)
- [x] System information page
  - CPU temperature, disk usage, IP address, version, battery, Auto-Hotspot
  - Log viewer (error + debug tabs, on-demand load, refresh) in Settings › System
  - [ ] Configure (one or multiple) WLANs
- [x] `run_npm_build` script
  - [x] Must consider `export NODE_OPTIONS=--max-old-space-size=512`
- [x] Upload audio files via WebUI (`fileserver` component, POST to port 8080; FAB button in Library)
